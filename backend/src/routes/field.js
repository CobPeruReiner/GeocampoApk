...
const { safeIdentifier, portfolioForUser, portfoliosForUser, guiColumns, formatClient } = require('../utils/field');
...
function dateValue(input) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(input)) throw Object.assign(new Error('Fecha inválida.'), { status: 400 });
  return input;
}
function weekRange(date) {
  const value = new Date(`${date}T12:00:00`);
  const day = value.getDay() || 7;
  const monday = new Date(value); monday.setDate(value.getDate() - day + 1);
  const sunday = new Date(monday); sunday.setDate(monday.getDate() + 6);
  const serialize = (item) => item.toISOString().slice(0, 10);
  return { referenceDate: date, startDate: serialize(monday), endDate: serialize(sunday) };
}
function visitState(code, description) {
  const normalized = String(code || 'PENDIENTE').toUpperCase();
  return { code: normalized, description: description || 'Pendiente', completed: ['VISITADO', 'GESTIONADO', 'CERRADO'].includes(normalized) };
}

router.get('/initial', async (req, res, next) => {
  try {
    const portfolios = await portfoliosForUser(req.user.id);
    res.json({ profile: req.user, portfolios, week: weekRange(dateValue(req.query.date)) });
...
router.get('/route', async (req, res, next) => {
  try {
    ...
    const params = [req.user.id, week.startDate, week.endDate, week.referenceDate];
    if (query) {
      filters.push("(tb.IDENTIFICADOR LIKE ? OR COALESCE(tb.DOCUMENTO, '') LIKE ? OR COALESCE(tb.NOMBRE, '') LIKE ? OR COALESCE(tb.NUMEROCUENTA, '') LIKE ?)");
      const pattern = `%${query}%`; params.push(pattern, pattern, pattern, pattern);
    }
    ...
    const [rows] = await pool.execute(`
      ...
      WHERE ${where}
      ORDER BY DATE(hrd.fecha_agendada), hrd.orden_visita, hrd.id_detalle LIMIT ?`, [portfolio.id_cartera, ...params, limit]);
    const items = rows.map((row) => ({ ...row, state: visitState(row.status_code, row.status_description), portfolio: { idTable: portfolio.id_table, idCartera: portfolio.id_cartera, name: portfolio.name } }));
    res.json({ portfolio, week, summary: { total: Number(summaryRows[0]?.total || 0), completed: Number(summaryRows[0]?.completed || 0) }, items });
  } catch (error) { next(error); }
});
...
router.get('/clients', async (req, res, next) => {
  try {
    ...
    const pattern = `%${query}%`;
    const [rows] = await pool.execute(`SELECT * FROM ${table} WHERE IDENTIFICADOR LIKE ? OR COALESCE(DOCUMENTO, '') LIKE ? OR COALESCE(NOMBRE, '') LIKE ? LIMIT 50`, [pattern, pattern, pattern]);
    res.json({ portfolio, items: rows.map((row) => formatClient(row, portfolio, fields)) });
  } catch (error) { next(error); }
});
...
router.get('/clients/:identifier', async (req, res, next) => {
  try {
    ...
    const fields = await guiColumns(portfolio.id_table);
    const table = safeIdentifier(portfolio.table_name);
    const [rows] = await pool.execute(`SELECT * FROM ${table} WHERE IDENTIFICADOR = ? LIMIT 1`, [req.params.identifier]);
    if (!rows[0]) return res.status(404).json({ message: 'Cliente no encontrado en la cartera asignada.' });
...
router.get('/clients/:identifier/history', async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.execute(`
      ...
      ORDER BY g.FECHA DESC, g.ID DESC LIMIT 100`, [req.params.identifier, portfolio.id_cartera]);
